	...
	public static void main(String[] args){
		...
		for(int i=0; i<args.length; i++){
			...
			if(Parser.parseCommonStatic(arg, a, b)){
				...
				outstream.println("Set overwrite to "+overwrite);
		...
		srt.processMT();
		if(MERGE_DUPLICATES){
			outstream.println("Merged "+srt.merged+" duplicates of "+srt.processed+" total.");
			if(srt.correctMerged>0 || srt.incorrectMerged>0){
				outstream.println("Merged "+srt.correctMerged+" reads from same origin (correct).");
				outstream.println("Merged "+srt.incorrectMerged+" reads from different origin (incorrect).");
			}
		}
...
//		assert(false) : fname1+", "+fname2+", "+outname+", "+prefix;
		...
		outstream.println("Started blockwriters");
		
		{
			...
			cris.returnList(ln.id, ln.list.isEmpty());
			outstream.println("Returned list");
			ReadWrite.closeStream(cris);
			outstream.println("Closed stream");
		}

		
		synchronized(this){this.notifyAll();}
		outstream.println("Notified all");
		
		finishWritingBlocks();
		outstream.println("Wrote blocks");
...
//		if(bwt2!=null){blockwriter2.addList(null);}
		...
		t.stop();
		outstream.println("Temp Write Time: "+t);
		t.start();
		...
		for(String key : keys){
			...
			if(b.added>MAX_BLOCKSIZE_TO_SORT){
				outstream.println("Skipping sorting for key "+key+" of size "+b.added);
...
//				outstream.println(key+"\t"+b.added);
				b.delete();
			}else{
				ArrayList<Read> list=b.readBlock();
				if(PRINT_BLOCKS){outstream.println(key+"\t"+list.size());}
				b.delete();
...
//		if(wt2!=null){wt2.addList(null);}
		...
		t.stop();
		total.stop();
		outstream.println("Final Sort + Write Time: "+t);
...
//		String key=sb.toString();
		Block b=table.get(key);
		if(b==null){
			//outstream.println("Created block "+key);
	...
	public void finishWritingBlocks(){
		outstream.println("Called finishWritingBlocks()");
	...
	private class Block{
		...
		public void writeBuffer(boolean close){
			...
			if(close){
//				outstream.println("Closing "+name+": "+ fname1+", "+fname2);
...
//			buffer.clear();
		...
		public void finishWritingBuffer(){
			//outstream.println("Writing block "+name);
			writeBuffer(true);
