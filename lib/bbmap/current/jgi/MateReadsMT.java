...
public class MateReadsMT {
	...
	public static void main(String[] args){
		MateReadsMT x=new MateReadsMT(args);
		x.process();
		...
		for(int i=0; i<args.length; i++){
			...
			if(Parser.parseCommonStatic(arg, a, b)){
				...
				if(b.equalsIgnoreCase("true") || b.equalsIgnoreCase("null")){
					b="";
					outstream.println("Note - no hash tables will be used as there are no gaps.");
					gap_G=new int[1][0];
				}else{
					String[] h=b.split("[;:]");
...
//				for(int m=0; m<gap.length; m++){outstream.println(Arrays.toString(gap[m]));}
//				assert(false);
...
//		assert(false) : MATE_BY_OVERLAP;
		...
		if(FILL_MIDDLE_INTERMEDIATE){
			if(!WRITE_INTERMEDIATE_JOINED){outstream.println("WRITE_INTERMEDIATE_JOINED forced to true.");}
			WRITE_INTERMEDIATE_JOINED=true;
		}
		if(WRITE_INTERMEDIATE_JOINED){
			if(!join_G){outstream.println("Final output forced to be joined reads.");}
			join_G=true;
...
//		assert(!FASTQ.PARSE_CUSTOM);
		...
		outstream.println("Detected "+Runtime.getRuntime().availableProcessors()+" hardware threads; using "+THREADS+" for main process.");
		long memory=(Runtime.getRuntime().maxMemory());
		outstream.println("Detected "+(memory/(1L<<20))+" MB available memory.");
//		outstream.println("PARSE_CUSTOM="+FASTQ.PARSE_CUSTOM);
...
//			outstream.println(tablebytes/1000000+", "+usable/1000000+", "+(tablebytes*3)/1000000);
			...
			outstream.println("\nAuto settings:");
			outstream.println("k:          \t"+k_G);
			outstream.println("cbits:      \t"+cbits_G);
//			outstream.println("matrixbits: \t"+matrixbits);
//			outstream.println("matrixbits2:\t"+matrixbits2);
			outstream.println("cells:      \t"+Tools.toKMG(totalcells_G));
		...
		if(FILL_MIDDLE_INTERMEDIATE || FILL_MIDDLE_FINAL){
			...
			outstream.println("MiddleTable: \tgap = "+middleTable.gap+"   \tmem = "+middleTable.mem()+"   \tused = "+String.format(Locale.ROOT, "%.3f%%",middleTable.usedFraction()*100));
		}
...
//			}
		...
		outstream.println("Joined:      \t"+sum+String.format((sum<10000 ? "       " : "   ")+"\t%.3f%%", sum*div));
		if(FASTQ.PARSE_CUSTOM){
			outstream.println("Correct:     \t"+correctCountTotal+String.format((correctCountTotal<10000 ? "       " : "   ")+"\t%.3f%%", correctCountTotal*div));
			outstream.println("Incorrect:   \t"+incorrectCountTotal+String.format((incorrectCountTotal<10000 ? "       " : "   ")+"\t%.3f%%", incorrectCountTotal*div));
		}
		outstream.println("Ambiguous:   \t"+ambiguousCountTotal+String.format((ambiguousCountTotal<10000 ? "       " : "   ")+"\t%.3f%%", ambiguousCountTotal*div));
		outstream.println("No Solution: \t"+noSolutionCountTotal+String.format((noSolutionCountTotal<10000 ? "       " : "   ")+"\t%.3f%%", noSolutionCountTotal*div));
		if(minInsert>0){outstream.println("Too Short:   \t"+tooShortCountTotal+String.format((tooShortCountTotal<10000 ? "       " : "   ")+"\t%.3f%%", tooShortCountTotal*div));}
		outstream.println("Avg Insert:          \t\t"+String.format(Locale.ROOT, "%.1f", (insertSumCorrectTotal+insertSumIncorrectTotal)*1d/(correctCountTotal+incorrectCountTotal)));
		if(FASTQ.PARSE_CUSTOM){
...
//			assert(outgood.contains("#") || sam || fq) : outgood;
			if(outgood.contains("#")){
				...
				out1=outgood;
				out2=null;
				if(!join){outstream.println("Writing joinable reads interleaved.");}
				else{outstream.println("Writing joinable reads joined.");}
			}
...
//			assert(outbad.contains("#") || sam || fq) : outbad;
			if(outbad.contains("#")){
				...
				out1=outbad;
				out2=null;
				outstream.println("Writing unjoinable reads interleaved.");
			}
			assert(!out1.equalsIgnoreCase(in1) && !out1.equalsIgnoreCase(in1));
			assert(out2==null || (!out2.equalsIgnoreCase(in1) && !out2.equalsIgnoreCase(in2)));
		...
		for(int i=0; i<kca.length; i++){
			kca[i].shutdown();
			outstream.println("Table "+i+":\tgap = "+kca[i].gap+"   \tmem = "+kca[i].mem()+"   \tused = "+String.format(Locale.ROOT, "%.3f%%",kca[i].usedFraction()*100));
//			printStatistics(kca[i]);
		}
		
		if(kca!=null && kca.length>0){
			thash.stop();
			outstream.println("Hash time:  "+thash);
		}
...
//		outstream.println("len = "+kca.length);
...
//				if(bestInsert==331){
//					good331++;
//					outstream.println(guess+", "+bestInsert+", id="+a.numericID+", "+Arrays.toString(rvector));
//				}else{
//					bad331++;
//					outstream.println(guess+", "+bestInsert+", id="+a.numericID+", "+Arrays.toString(rvector));
//				}
...
//			outstream.println(overlap+", "+bestOverlap+", "+bestGood+", "+bestBad+", "+ambig);
...
//				outstream.println((abases.length+bbases.length-bestOverlap)+", "+ambig+", "+rvector[0]+", "+bestGood+", "+bestBad+", "+a.insertSize());
//			}
...
//		outstream.println(insert+", "+overlap+", "+start1+", "+start2);
	...
	public static int scoreIP(long[] half1, long[] half2, int insert, int pivot, KCountArray kca, int[] rvector, final int badlimit){
		...
		if(verbose){
			outstream.println("ScoreIP.  Insert:  "+insert+", gap="+kca.gap+", badlimit="+badlimit);
		}
...
//					if(verbose){outstream.print("("+Long.toHexString(half1[i])+","+Long.toHexString(half2[j])+","+Long.toHexString(key)+","+x+")");}
				}
			}
			if(verbose){
...
//		outstream.println(k+", "+bases.length+", "+half.length+", offset="+offset);
		
		int len=0;
		long kmer=0;
		for(int i=0, j=i-k+1; i<bases.length; i++, j++){
//			outstream.println(len+", "+i+", "+j);
			byte b=bases[i];
			...
			while(reads!=null && reads.size()>0){
				...
				for(Read r1 : reads){
					...
					if(!remove){
						...
						//				outstream.println("True Insert: "+r.insertSize());
						...
						if(r2==null){
							...
							//							outstream.println("Bad insert size for pre-joined read "+r.numericID+": len="+r.length()+", insert="+r.insert()+", actual="+trueSize);
							//						}
							bestVotes=1;
							ambig=false;
