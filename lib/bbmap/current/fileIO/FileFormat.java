...
//				assert(a.startsWith("in")) : "Unknown parameter "+arg;
		...
		if(full){
			...
			for(String fname : files){
				test(fname, full, true);
			}
		}
		
	}
	
	private static void test(String fname, boolean full, boolean forceFileRead){
...
//		assert(false) : ffName+"\n"+ffContent;
		...
		if(full){
			...
			{
				cris=ConcurrentReadInputStream.getReadInputStream(-1, true, ff, null);
				cris.start();
			}
		}
	}
	
	private static void fullTest(FileFormat ff){
